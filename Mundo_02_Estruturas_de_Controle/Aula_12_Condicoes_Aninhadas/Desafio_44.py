...
#     print(f'''\nVocê escolheu a {cores['verde']}opção (1){cores['limpa']} para pagar à vista no dinheiro ou cheque.
...
#     if parcelas < 3:
#         print(f"{cores['vermelho']}ERRO. Digite um parcelamento de no mínimo 3 vezes.{cores['vermelho']}")
#     else:
...
# else:
#     print(f"Você digitou a opção {cores['vermelho']}{opcoes_de_pagamento}{cores['limpa']}. {cores['vermelho']}Essa opção não é válida.{cores['limpa']}")

'''
Alternativa feita pelo professor
'''

# Cabeçalho formatado e centralizado
print('{:=^40}'.format(' LOJAS GUANABARA '))

# Entrada do preço das compras
preco = float(input("Preço das compras: R$ "))

# Menu com as formas de pagamento
...
# Exibição do valor final (caso a opção seja válida)
